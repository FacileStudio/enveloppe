...
One event per sealed block of AI-agent activity. Emitted by Jardin, consumed by Sablier.
...
Nothing breaks today because Jardin is the only producer and does have emails, but it
...
| | Go | TypeScript |
|---|---|---|
| Enum values | Typed constants (`AppJardin`) | String-literal unions, no constants |
