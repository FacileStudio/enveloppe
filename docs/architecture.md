...
```
Jardin (Go)  ──┐ enveloppe.Event[AgentSession]
...
object it does not own. Only two repos import this module today (Sablier and Jardin, both Go),
...
| `time_entry` | Sablier | Jardin **never** emits this, though it produces the raw activity |
| `agent_session` | Jardin | One event per sealed block of AI-agent activity |
...
every future pair. Jardin emits `agent_session` — machine, agent, branch, `user_email`,
start/stop, token totals — for a sealed block of agent activity. Sablier listens and
materializes those as time entries. Jardin does not emit `time_entry` because that object
...
both languages. Only Jardin emits `agent_session` and Jardin does have emails, so nothing is
